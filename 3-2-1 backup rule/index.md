# 3-2-1 backup rule
- [What it is](#what-it-is)
- [Why it is important](#why-it-is-important)
- [Example of storage](#example-of-storage)
- [Example of backup](#example-of-backup)

## What it is

The 3-2-1 backup rule is a data backup strategy that ensures data is protected against loss or corruption. It involves keeping three copies of your data, with one copy stored offsite. This provides redundancy and ensures data availability in case of hardware failure or other disasters.

## Why it is important

Data loss can have serious consequences, including financial losses, legal liabilities, and reputational damage. Implementing the 3-2-1 backup rule helps ensure data availability and reduces the risk of data loss.

## Example of storage

First is the device you want to backup from is considered as the first copy. The second copy should be stored in a different device as such as NAS, USB drive, different hard drive, or external hard drive. The third copy should be stored offsite, such as in a cloud storage service or a different location.

## Example of backup

Here is an example of a backup strategy that follows the 3-2-1 rule:

- First copy: Your phone
- Second copy: A NAS device in your home
- Third copy: A cloud storage service (e.g. Google Drive, iCloud, or OneDrive)
